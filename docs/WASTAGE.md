...
Staff report goods name, damage date, quantity measured in kilograms (up to three decimal places) or whole individual units, damage details and 1–5 JPG/PNG photos totalling at most 15 MB. The authenticated reporter's ID and name are recorded automatically. Original submissions are immutable.
...
In-app notifications alert owners to new reports and proof, and the reporting staff member to instructions, requests for more evidence and acceptance. The bell refreshes every 30 seconds while the app is visible. Notifications are persisted for the user's next login. Optional free-tier Brevo email notifications follow the same events.
...
Photos use the existing private object storage. Migration `f920_wastage` adds wastage, evidence and notification tables; `fb22_wastage_quantity_unit` preserves existing quantities as kilograms and adds individual units. Deployment runs the normal Alembic upgrade; no existing business records are removed. This module records wastage incidents and does not deduct inventory, since a stock ledger has not yet been implemented.
